...
function Client(server, nick, opt) {
    ...
    self.opt = {
        ...
        stripColors: false,
        timeout: 120000 // Recommend implementing recurring CTCP PING in the client to avoid socket timeouts
    };
...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    self.conn.setTimeout(self.opt.timeout);
    ...
    self.conn.addListener("end", disconnectedCallback);
    self.conn.addListener("close", disconnectedCallback);
    self.conn.addListener("timeout", disconnectedCallback);
